...
DOCKERFILE_PATH="${DIR}/../build/docker/compose.yaml"
...
if [[ "$CMD_ARG" == "$SETUP_ARG" ]]; then
    ...
    touch "${PRISMA_FOLDER}/.env"
    # add development settings
    echo "DATABASE_DB=dev-ah" >> "${LOCAL_FOLDER}/.env"
    echo "DATABASE_USER=backend-ah" >> "${LOCAL_FOLDER}/.env"
    echo "DATABASE_PASSWORD=backend-ah" >> "${LOCAL_FOLDER}/.env"
